            ...
            if (analysis["extensions"]) {
				...
				if (config["input"] && config["input"]["extensions"]) {
					...
					/*if (ext["activate_mk_analysis"]) {
						...
						if (want_mk && ext_input["mk_constraints"]) {
...
int main(int argc, char** argv)
{
	...
	want_precedence = options.get("precedence_file");
	...
	want_aborts = options.get("abort_file");
	...
	want_mutexes = options.get("excl_file");
	...
	platform_defined = options.get("platform_file");
	if (platform_defined) {
